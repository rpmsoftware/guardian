...
exports.CoastGuard = function init(config, mailerConfig, otherConfigs) {
    ...
    this.runAllTests = function() {
        console.log('\CoastGuard status: starting - ' + config.name + '(' + config.url + ')');
        var testsDonePromise = new Promised.Deferred();
        ...
        var promises = fs.readdirSync(path)
            .filter(function(file) {
                ...
                var promise = test.runTest(config, otherConfigs);
                return promise;
            });

        Promised.all(promises).then(
            function(results) {
                console.log('\CoastGuard status: done - ' + config.name + '(' + config.url + ')');
                ...
                Mailer.sendEmail({
                    ...
                    content: report
                }, mailerConfig, testsDonePromise);
            }
        );

        return testsDonePromise;
    }
};