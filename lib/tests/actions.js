...
ActionTest = __class('Action', {
    ...
    checkNotificationEmails: function() {
        var checkDone = new Deferred();
        var client = new ImapClient.default('imap.gmail.com', 993, {
            auth: {
                user: this.otherConfigs.notification_email.address,
                pass: this.otherConfigs.notification_email.password
            },
            logLevel: 1000
        });
        client.onerror = function(error) {
            console.log('Notification email IMAP error', error);
            this.error('Notification email', 'An error occurred while fetching the email from ' + this.config.notification_email);
            this.finishEmailCheck(client, checkDone);
        }.bind(this);

        var expectedSubject = this.config.name + ' Actions ' + this.getRaw('YesterdayForm').Number;
        client.connect().then(function() {
            client.listMessages('INBOX', '1:*',['uid', 'body[header.fields (content-type)]', 'body[header.fields (subject)]', 'body[text]']).then(function(emails) {
    ...
    checkEmail: function(email, client, checkDone) {
        ...
        var body = email['body[text]'].trim().split('--' + boundary);
